...
public class GuiButton extends GuiComponent implements Clickable {
    ...
    protected ItemStack notEnableButton;
    ...
    public GuiButton(GUI gui, String id, int slot) {
        ...
        notEnableButton = guiLibs.getItemBuilder(guiLibs.getAdapter().getRedGlassPane()).setDisplayName("").addItemTag("not-enable", true).build();
    }
    ...
    public void render(GuiRenderer renderer) {
        updateListeners.run();
        renderer.setSlot(0, guiLibs.getItemBuilder(enable ? displayItem : notEnableButton).addItemTag("component-id", getFullId()).addItemTag("real-item", displayItem).build());
    }
